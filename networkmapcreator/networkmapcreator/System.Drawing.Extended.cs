...
namespace System.Drawing.Extended
{
    ...
    public class ExtendedGraphics
    {
        ...
        {
            DrawRoundRectangle(pen, rect.X, rect.Y, rect.Width, rect.Height, radius);
        }
           ...
           float radius)
        {
            ...
            if (radius <= 0.0F || baseRect.Height <= 0 || baseRect.Width <= 0)
            {
        ...
        public void DrawTwoToneLine(Color c1, Color c2, int width, int space, Point a, Point z)
        {
            mGraphics.DrawLine(new Pen(c1, width), a, z);

            var p = new Pen(c2, width);
            float[] pattern = { space, space };
            p.DashPattern = pattern;

            mGraphics.DrawLine(p, a, z);
        }

        public void DrawTwoToneBezier(Color c1, Color c2, int width, int space, Point a, Point b, Point c, Point d)
        {
            mGraphics.DrawBezier(new Pen(c1, width), a, b, c, d);

            var p = new Pen(c2, width);
            float[] pattern = { space, space };
            p.DashPattern = pattern;

            mGraphics.DrawBezier(p, a, b, c, d);
        }
    }
}